// -------------------------------------------------------------------------------------------------
// Configuracoes e construtores de graficos (Chart.js)
// -------------------------------------------------------------------------------------------------
// Resumo: paleta, plugins auxiliares e criacao dos graficos do comparador e do ciclo de vida.
// -------------------------------------------------------------------------------------------------
export const BLUE_PALETTE = ["#0b5c8a", "#0d6fb2", "#1380c9", "#1992dd", "#3aa9f2"];

export const withAlpha = (hex, alpha = "b3") => `${hex}${alpha}`;

// Plugins compartilhados para graficos do comparador e ciclo de vida
// -------------------------------------------------------------------------------------------------
// Plugins auxiliares (labels e crosshair)
// -------------------------------------------------------------------------------------------------
  ...
  afterDatasetsDraw(chart, args, opts) {
    ...
    chart.data.datasets.forEach((dataset, datasetIndex) => {
      ...
      meta.data.forEach((bar, index) => {
        // Stacked: desenha no topo do stack visivel
        if (opts.stacked) {
...
// Plugin de linhas de intersecao: aceita alvo em y (encontra x) ou alvo em x (encontra y) no 1o dataset.
...
// -------------------------------------------------------------------------------------------------
// Graficos do comparador (composicao completa)
// -------------------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------
// Graficos do comparador simultaneo (versao enxuta)
// ---------------------------------------------------------------------------------------
export function createComparadorChartsLite({
  ...
  const consumoChart = new Chart(consumoCanvas, {
    ...
    data: {
      ...
      datasets: [
        {
          label: `Consumo${lifeLabel || " (anual)"} (kWh)`,
          data: consumos,
          backgroundColor: colorScaleConsumo,
        },
      ],
    },
    options: {
        ...
        tooltip: {
          callbacks: {
            title: (items) => {
              ...
              return tooltipLabelsConsumo?.[idx] || "";
            },
            label: (ctx) => `${ctx.dataset.label}: ${formatNumber(ctx.parsed.y, 0)} kWh`,
          },
        },
  ...
  const custoChart = new Chart(custoCanvas, {
    type: "bar",
    data: {
      labels: labelsCusto,
      datasets: [
        {
          label: `Custo de Energia${lifeLabel || " (anual)"}`,
          data: custosEnergia,
          backgroundColor: colorScaleCusto,
        },
      ],
    },
    options: {
...
// -------------------------------------------------------------------------------------------------
// Graficos de ciclo de vida (confiabilidade)
// -------------------------------------------------------------------------------------------------
